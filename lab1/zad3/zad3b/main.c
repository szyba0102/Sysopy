...
int main(int argc, char *argv[]){
    ...
    if (handle == NULL)
    {
        fprintf(stderr, "Error with opening dynamic library");
        return -1;
    }

    int (*wc_files)(int tab_files_size, char **tab_files) = dlsym(handle, "wc_files");
    int (*rewrite)() = dlsym(handle, "rewrite");
    int (*remove_block)(int inx) = dlsym(handle, "remove_block");
    int (*create_table)(int size) = dlsym(handle, "create_table");
